...
import BecomeVolunteerModal from '../Shared/BecomeVolunteerModal';
...
const DonorSidebar = () => {
  ...
  return (
    ...
    <aside className={`fh-sidebar ${isOpen ? 'is-open' : ''}`}>
      ...
      <div className="fh-role-box">
        <div className="fh-role-inner">
          ...
          {/* Single button does both jobs:
              - User is Donor only       → opens "Become a Volunteer" modal
              - User is Donor + Volunteer → switches to Volunteer dashboard */}
          {(user?.roles || []).includes('Volunteer') ? (
            <button
              className="fh-role-switch"
              title="Switch to Volunteer"
              onClick={() => {
                const updated = { ...user, current_role: 'Volunteer', role: 'Volunteer' };
    ...
    {/* Become-a-Volunteer modal — opened from the sidebar's role-switch button */}
    <BecomeVolunteerModal
      open={showVolunteerModal}
      onClose={() => setShowVolunteerModal(false)}
      userId={user?.user_id}
      onSuccess={(roles) => {
        // Update localStorage so future renders / ProtectedRoute see Volunteer.
        // Legacy `role` is kept in sync with `current_role` so the route guard
        // (which falls back to user.role) still passes.
        ...
        localStorage.setItem('feedhope_user', JSON.stringify(switched));
        navigate('/volunteer-dashboard');
      }}
    />
    </>
