...
const ReceiverSidebar = ({ activePage }) => {  // removed unreadCount from props
    ...
    return (
        ...
        <aside className={`rsb-sidebar ${isOpen ? 'is-open' : ''}`}>
            ...
            <div className="rsb-role-row">
                ...
                {/* Single button does both jobs:
                    - User is Receiver only      → opens "Become a Volunteer" modal
                    - User is Receiver + Volunteer → switches to Volunteer dashboard */}
                {(user?.roles || []).includes('Volunteer') ? (
                    <button
                        className="rsb-role-switch"
        ...
        <BecomeVolunteerModal
            ...
            userId={userId}
            onSuccess={(roles) => {
                // Keep legacy `role` in sync with `current_role` so the route
                // guard (which falls back to user.role) lets us through.
