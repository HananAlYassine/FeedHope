...
const MapModal = ({ open, onClose, donor, receiver, distanceKm, foodName }) => {
    ...
    useEffect(() => {
        ...
        ensureLeafletLoaded().then((L) => {
            ...
            // Override Leaflet's default prefix (which includes a Ukrainian flag
            // emoji since v1.9) with a Lebanon-themed FeedHope label. Uses an
            // inline SVG instead of an emoji because Windows doesn't render
            // flag emojis as actual flags — only the country letters.
            if (map.attributionControl) {
                const lebanonFlagSvg =
                    "<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 30 18' " +
                    "width='16' height='10' style='vertical-align:middle;margin-right:5px;border:1px solid #ccc;'>" +
                    "<rect width='30' height='18' fill='#fff'/>" +
                    "<rect width='30' height='4.5' fill='#EE161F'/>" +
                    "<rect y='13.5' width='30' height='4.5' fill='#EE161F'/>" +
                    "<polygon points='15,6.5 11.5,12 18.5,12' fill='#00A651'/>" +
                    "<rect x='14.7' y='11.5' width='0.6' height='1.2' fill='#00A651'/>" +
                    "</svg>";
                map.attributionControl.setPrefix(lebanonFlagSvg + 'FeedHope');
            }
