...
const DonorMyOffers = () => {
    ...
    // Fixed-position coordinates for the dropdown so it escapes the table
    // wrapper's overflow clip (otherwise the menu on the last row gets cut off).
    ...
    // Locked = donor can no longer edit or cancel. Once a receiver has
    // accepted (or a volunteer is delivering, or it's already delivered/
    // expired), the offer is committed and shouldn't be modified.
    const isLocked = (status) =>
        ['accepted', 'in_delivery', 'delivered', 'expired'].includes(
            (status || '').toLowerCase()
        );

    // Toggle Action Menu — also computes a fixed position so the dropdown
    // can render outside the (clipped) table wrapper, and flips above the
    // button when there isn't enough room below (e.g. last row in the table).
    const toggleActionMenu = (offerId, event) => {
        event.stopPropagation();
        if (activeActionMenu === offerId) {
            setActiveActionMenu(null);
            setMenuPos(null);
            return;
        }
        const rect = event.currentTarget.getBoundingClientRect();
        const dropdownHeight = 110; // approx — two buttons + padding
    ...
    const handleDeleteClick = async (offerId) => {
        setActiveActionMenu(null);
        setMenuPos(null);
        // Defense in depth — UI already disables the button when locked,
        // but status may change between render and click (real-time poll).
        const target = offers.find(o => o.offer_id === offerId);
        if (target && isLocked(target.status)) {
            alert(`This offer is "${target.status}" and can no longer be cancelled.`);
            return;
        }
    ...
    const handleEditClick = async (offer) => {
        setActiveActionMenu(null);
        setMenuPos(null);
        // Defense in depth — UI already disables this button when locked.
        if (isLocked(offer.status)) {
            alert(`This offer is "${offer.status}" and can no longer be edited.`);
            return;
        }
    ...
    // Close dropdown when clicking outside or scrolling (the dropdown is
    // fixed-positioned so its position would otherwise get out of sync).
    useEffect(() => {
        const closeMenu = () => {
            setActiveActionMenu(null);
            setMenuPos(null);
                                            ...
                                            <tr key={offer.offer_id}>
                                                ...
                                                <td className="actions-cell">
                                                    ...
                                                    <div className="action-menu-wrapper" onClick={(e) => e.stopPropagation()}>
                                                        ...
                                                        {activeActionMenu === offer.offer_id && menuPos && (
                                                            <div
                                                            ...
                                                            >
                                                                <button
                                                                    ...
                                                                    disabled={isLocked(offer.status)}
                                                                    title={isLocked(offer.status)
                                                                        ? `Cannot edit — offer is ${offer.status}`
                                                                        : 'Edit offer'}
                                                                >
                                                                ...
                                                                <button
                                                                    ...
                                                                    disabled={isLocked(offer.status)}
                                                                    title={isLocked(offer.status)
                                                                        ? `Cannot cancel — offer is ${offer.status}`
                                                                        : 'Cancel offer'}
                                                                >
