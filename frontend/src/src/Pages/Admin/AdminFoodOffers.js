...
const STATUS_BADGE = {
    ...
    delivery_accepted: { label: 'Delivery Accepted', cls: 'afo-badge--delivery-accepted' },
};
...
const AdminFoodOffers = () => {
    ...
    // Sends a REQUEST that the volunteer can accept or reject — admins
    // can no longer directly bind a volunteer without their consent.
    ...
    const handleAssignSubmit = async () => {
        ...
        try {
            // Pull admin_id from localStorage so the request stamps the right admin
            ...
            const res = await fetch('http://localhost:5000/api/admin/request-volunteer', {
                ...
                body: JSON.stringify({
                    offerId: assignModal,
                    volunteerUserId: selectedVol,
                    message: assignMessage.trim() || null,
                    adminId: stored.admin_id || null,
                }),
            });
            const data = await res.json();
            if (!res.ok) { showToast(data.error || 'Failed to send request.', 'error'); return; }
            showToast('Request sent to volunteer — awaiting their response.');
            setAssignModal(null);
            setAssignMessage('');
            fetchOffers();
        } catch {
            ...
            {assignModal && (
                <div className="afo-modal-backdrop" onClick={() => setAssignModal(null)}>
                    <div className="afo-modal afo-modal--sm" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="afo-modal-body">
                            <p className="afo-modal-hint">
                                Send a delivery request to a volunteer. They can accept or reject it from their dashboard.
                            </p>
                            <select
                            ...
                            <textarea
                                className="afo-select afo-select--full"
                                style={{ marginTop: 10, minHeight: 70, padding: '8px 10px', fontFamily: 'inherit' }}
                                placeholder="Optional message to the volunteer (e.g. urgency, special instructions)…"
                                value={assignMessage}
