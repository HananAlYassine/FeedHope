...
const ProtectedRoute = ({ children, requiredRole }) => {
    ...
    // Allow if the required role is anywhere in the user's role set:
    // - new `roles` array (dual-role users may have both Donor + Volunteer)
    // - `current_role` (most-recently-active role)
    // - legacy `role` field (older sessions before the dual-role refactor)
    if (requiredRole) {
        const userRoles = Array.isArray(user.roles) ? user.roles : [];
        const hasRole =
            userRoles.includes(requiredRole) ||
            user.current_role === requiredRole ||
