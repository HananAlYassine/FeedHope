...
// Hardcoded for now, but usually comes from Auth Context / LocalStorage
const CURRENT_DONOR = { donor_id: 101, user_id: 1001 };
...
const DonorNewOffers = () => {
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            try {
                // Matches the GET route in index.js
    ...
    const handleSubmit = async (e) => {
        ...
        const dietaryText = dietarySelections.length ? dietarySelections.join(', ') : 'No specific dietary flags';

        // USE FORMDATA FOR MULTIPART/FORM-DATA (Text + File)
        ...
        formData.append('donorId', CURRENT_DONOR.donor_id);
        ...
        try {
            ...
            if (response.ok) {
                setNotification({ message: `✅ Offer created! (ID: ${result.offerId})`, type: 'success' });
                // Optional: resetForm() here to clear the page after success
            } else {
                    ...
                    <div className="form-card">
                        ...
                        <form className="form-body" onSubmit={handleSubmit}>
                            ...
                            <div className="input-group">
                                ...
                                <input type="text" value={foodName} onChange={(e) => setFoodName(e.target.value)} placeholder="e.g. Leftover Pasta & Salads" required />
                            </div>

                            {/* Description */}
                            <div className="input-group">
                                <label><DescriptionIcon /> Description</label>
                                <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Describe food items, packaging, and any special instructions..."></textarea>
                            </div>
                            ...
                            <div className="input-group">
                                ...
                                {imagePreview && <div className="image-preview"><img src={imagePreview} alt="Preview" style={{ maxHeight: '200px', marginTop: '10px', borderRadius: '8px' }} /></div>}
                            </div>
                            ...
                            <div className="row-2cols">
                                ...
                                <div className="input-group">
                                    ...
                                    <input type="number" step="0.1" value={quantityKg} onChange={(e) => setQuantityKg(e.target.value)} placeholder="e.g. 5.5" required />
                                </div>
                            </div>
                            ...
                            <div className="row-2cols">
                                <div className="input-group">
                                    ...
                                    <input type="number" value={numPersons} onChange={(e) => setNumPersons(e.target.value)} placeholder="e.g. 12" />
                                </div>
                            </div>
                            ...
                            <div className="input-group">
                                ...
                                <div className="dietary-group">
                                    <div className="dietary-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                                        {DIETARY_OPTIONS.map(opt => (
                                            <label key={opt} className="dietary-check">
                                                <input type="checkbox" checked={dietarySelections.includes(opt)} onChange={() => handleDietaryChange(opt)} /> {opt}
                                            </label>
                                        ))}
                                    </div>
                                </div>
                            ...
                            <div className="form-actions" style={{ display: 'flex', gap: '10px', marginTop: '20px' }}>
                                <button type="button" className="btn-cancel" onClick={resetForm} disabled={isSubmitting}><CancelIcon /> Cancel</button>
                        ...
                        {notification.message && (
                            <div className={`notification ${notification.type}`} style={{ marginTop: '15px', padding: '10px', borderRadius: '5px', backgroundColor: notification.type === 'error' ? '#ffebee' : '#e8f5e9' }}>
                                {notification.message}
                                <button onClick={() => setNotification({ message: '', type: '' })} style={{ float: 'right', background: 'none', border: 'none', cursor: 'pointer' }}>×</button>
                            </div>
                        )}
                    </div>
                </div>
