...
const ReceiverHistory = () => {
    ...
    // Real-time: silently re-fetch every 3s so an offer just taken by a
    // volunteer appears here instantly (and status badges update as the
    // volunteer starts / completes the delivery).
    useEffect(() => {
        if (!userId) return;
                    ...
                    <table className="rao-table">
                        ...
                        <tbody>
                            {history.length === 0 ? (
                                ...
                                history.map(item => {
                                    const isCompleted = ['delivered', 'completed'].includes(item.delivery_status);
                                    const statusLabel =
                                        item.delivery_status === 'delivery_accepted' ? 'Volunteer Accepted' :
                                        item.delivery_status === 'in_delivery'       ? 'In Delivery' :
                                        'Delivered';
                                    const statusClass =
                                        isCompleted                                  ? 'rao-status-delivered' :
                                        item.delivery_status === 'in_delivery'       ? 'rao-status-accepted'  :
                                                                                        'rao-status-accepted';
                                    return (
                                        <tr key={item.offer_id}>
                                            ...
                                            <td data-label="Status">
                                                <span className={`rao-status-badge ${statusClass}`}>
                                                    {statusLabel}
                                                </span>
                                            </td>
                                            <td data-label="Delivered Date">
                                                {isCompleted ? item.delivered_date : '—'}
                                            </td>
                                            <td data-label="Feedback">
                                                {item.feedback_given ? (
                                                    ...
                                                    <button
                                                        ...
                                                        onClick={() => isCompleted && openFeedbackModal(item)}
                                                        disabled={!isCompleted || actionLoading === item.offer_id}
                                                        title={isCompleted
                                                            ? 'Rate the donor and the volunteer'
                                                            : 'Available once the volunteer marks the offer as delivered'}
                                                    >
