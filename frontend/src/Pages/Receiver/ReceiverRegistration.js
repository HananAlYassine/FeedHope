...
/* ═══════════════════════════════════════════════════════════
    MAIN — RECEIVER REGISTRATION
═══════════════════════════════════════════════════════════ */
const ReceiverRegister = () => {
    const navigate = useNavigate();

    const [formData, setFormData] = useState({
        organizationName: '', email: '', phone: '', orgType: '', otherOrgType: '',
        street: '', city: '', country: 'Lebanon', latitude: '', longitude: '',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch('http://localhost:5000/api/register/receiver', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ ...formData, orgType: finalOrgType }),
            });
                ...
                <div className="reg-card-container">
                        ...
                        <div className="reg-hero-content">
                            ...
                            <div>
                                <div className="glass-badge" style={{ marginBottom: '20px' }}>
                                    ...
                                    <p style={{ margin: '5px 0 0', fontSize: '14px', opacity: 0.9 }}>Meals Rescued Across 500+ Cities</p>
                                </div>
                                <div className="glass-badge">
                                    <FormatQuote style={{ fontSize: '30px', color: '#10b981' }} />
                                    <p style={{ fontStyle: 'italic', fontSize: '13px', margin: '5px 0' }}>
                                        "FeedHope helped us connect with local restaurants and feed 200 families weekly."
                                    </p>
                                    <strong style={{ fontSize: '12px' }}>- Maria Santos</strong>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="reg-form-section">
                        ...
                        <div className="role-pills-nav">
                            <button type="button" className="role-nav-btn" onClick={() => navigate('/donor-registration')}><Storefront fontSize="small" /> Donor</button>
                            <button type="button" className="role-nav-btn active-pill"><VolunteerActivism fontSize="small" /> Receiver</button>
                        ...
                        <form className="auth-input-grid" onSubmit={handleSubmit}>
                            <div className="auth-input-group full-width-field">
                                ...
                                <input type="text" name="organizationName" value={formData.organizationName} onChange={handleChange} className="auth-input-field" placeholder="Enter name" required />
                            </div>
                            ...
                            <div className="auth-input-group">
                                ...
                                <input type="number" step="any" name="latitude" value={formData.latitude} onChange={handleChange} className="auth-input-field" placeholder="Range: -90 to 90" required />
                            </div>
                            <div className="auth-input-group">
                                <label>Longitude <LocationOn style={{ fontSize: '12px' }} /></label>
                                <input type="number" step="any" name="longitude" value={formData.longitude} onChange={handleChange} className="auth-input-field" placeholder="Range: -180 to 180" required />
                            </div>
                            <div className="auth-input-group">
                                <label>Organization Type</label>
                                <select name="orgType" className="auth-select-field" value={formData.orgType} onChange={handleChange} required>
                            ...
                            {formData.orgType === 'Other' && (
                                <div className="auth-input-group full-width-field">
                                    <label>Specify Organization Type</label>
                            ...
                            <button type="submit" className="submit-reg-button full-width-field">Create Receiver Account</button>
