...
// ── LocalStorage helpers for admin-hidden notifications ──
...
const AdminNotifications = () => {
    ...
    const handleDeleteAllConfirm = async () => {
        ...
        try {
            // 1. Get all currently displayed notification IDs
            const currentIds = notifications.map(n => n.notification_id);

            // 2. Save them to localStorage hidden list
            if (currentIds.length) {
                addHiddenIds(currentIds);
            }
            ...
            window.dispatchEvent(new Event('notification-read'));
            showToast('All notifications removed from view (still stored).');
        } catch (err) {
            showToast('Failed to clear notifications from view.', 'error');
        } finally {
    ...
    // ── Confirmed: "Delete Single" – only hides one notification from UI ──
    const handleDeleteOneConfirm = async () => {
        if (!deleteSingleTarget) return;
        setActionLoading(true);
        try {
            // 1. Add this ID to localStorage hidden list
            addHiddenId(deleteSingleTarget);
            ...
            window.dispatchEvent(new Event('notification-read'));
            showToast('Notification removed from view (still stored).');
        } catch {
